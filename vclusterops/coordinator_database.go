...
func (vdb *VCoordinationDatabase) addHosts(hosts []string) error {
	...
	vNodeMap := vdb.genVnodeMap()
	for _, host := range hosts {
		vNode := MakeVCoordinationNode()
		name, ok := util.GenVNodeName(vNodeMap, vdb.Name, totalHostCount)
		if !ok {
...
// genVnodeMap generates a vnode map from HostNodeMap.
func (vdb *VCoordinationDatabase) genVnodeMap() map[string]string {
...
// doNodesExist returns true if a certain number(passed as parameter) of the given nodes exist in
// database.
func (vdb *VCoordinationDatabase) doNodesExist(nodes []string, targetCount int) bool {
	...
	return len(dupHosts) == targetCount
}
